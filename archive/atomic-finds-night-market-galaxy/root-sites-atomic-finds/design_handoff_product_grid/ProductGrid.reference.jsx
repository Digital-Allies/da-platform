...
function ProductGridRef({
  ...
  return (
    <div {...props} style={{ ...props.style }}>
      {title && (
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--fs-h1)', color: 'var(--celestial-yellow)', textShadow: 'var(--glow-md)', textAlign: 'center', margin: '0 0 var(--space-10)' }}>{title}</h2>
      )}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-6)' }}>
        {products.map((p) => (
          <ProductGridCardRef key={p.id} {...p} />
        ))}
      </div>
    </div>
  );
