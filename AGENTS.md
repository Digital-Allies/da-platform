...
- **`tools/build-workflows` IS a workspace member** (`@ally-cms/design-system` resolves via a symlink at `tools/build-workflows/node_modules/@ally-cms/design-system`). Confirmed against a real Vercel deployment (2026-09-20): npm's workspace resolution walks up from the `tools/build-workflows` Root Directory to find the workspace root even without any monorepo-specific Vercel project settings — `npm install --legacy-peer-deps` and `next build` both succeed. Nothing in `tools/build-workflows/src` actually imports from `@ally-cms/design-system` yet — that migration (replacing ad hoc file copies with real imports) is still a deliberate future step.
