...
- **CTA**: right-aligned pill button/link, resolved from `selling_state` (never hard-coded "Buy Now"): `listing`/`checkout` **with `external_url` set** → opens that link in a new tab (`target="_blank" rel="noopener noreferrer"`), default label "View Listing"/"Purchase Options". Everything else — `inquiry`, `direct`, **or a `listing`/`checkout` product with no URL** — → links to `#contact` in the current tab, default label "Ask About This Item" (a URL-less product never keeps a listing/checkout label, which would promise a link that isn't there). `cta_label` overrides the default label for any state. 11px/700/uppercase, `--celestial-yellow` text, 1px border, pill radius.
