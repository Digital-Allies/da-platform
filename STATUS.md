...
**Last updated:** 2026-09-20 — Claude Code: full documentation consolidation, Atomic Finds "Night Market Galaxy" brand archived. See below.
...
- **`packages/` is now a real npm workspace** — root `package.json` added (`workspaces: ["packages/*"]`), `packages/design-system/package.json` added (`@ally-cms/design-system`). New rule (see `AGENTS.md` "Packages" section): one real package per client, created fresh when that client is actually onboarded — not by relocating an old reference folder — and each package's resources need strict approval before another session touches them. `tools/build-workflows` is NOT yet a workspace member; wiring it in for real imports (vs. today's ad hoc file copies) is a deliberate next step requiring an `npm install` + build check, not done today. MCP-per-client-package (a possible future free upsell) was raised and explicitly deferred — no design work done.
